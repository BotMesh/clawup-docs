...
│  │  ↕ MCP 协议                     │        │
│  │  mcporter bind → 签名桥 Tool      │        │
│  └──────────────────────────────────┘        │
│                                              │
│  ┌─ MetaMask 签名桥 MCP Tool 容器 ──┐        │
│  │  localhost:18790                  │        │
...
- **start_bot**：返回 202 + job_id，后台执行 provisioner start + tool rebind
...
│   ├── mcporter_bind.sh         — MCP Tool 绑定脚本
...
| 签名桥 MCP Tool (`metamask-bridge/`) | ✅ 已实现 |
